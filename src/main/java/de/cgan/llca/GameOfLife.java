...
public class GameOfLife extends JFrame {
	...
	private void performStep() {
		...
		for (int row = 0; row < ROWS; row++) {
			for (int col = 0; col < COLS; col++) {
				...
				if (grid[row][col].getBackground() == ALIVE) {
					if (rule.getSurviveAt().contains(Integer.valueOf(numNeighbors))) {
					...
					if (rule.getBornAt().contains(Integer.valueOf(numNeighbors))) {
	...
	public static void main(String[] args) throws IOException {
		...
		ruleProperties.load(GameOfLife.class.getResourceAsStream("/rules.properties"));
		String ruleName = args.length < 1 ? "Life" : args[0];
		final Rule gameOfLifeRule = Rule.createFromString(ruleProperties.getProperty(ruleName));

		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				new GameOfLife(gameOfLifeRule);
			}
		});
	}
}
